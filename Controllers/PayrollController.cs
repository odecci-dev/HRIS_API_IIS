...
namespace API_HRIS.Controllers
{
    ...
    public class PayrollController : ControllerBase 
    {
        ...
        [HttpPost]
        public async Task<IActionResult> ComputePayslipAdmin(EmployeeFiltersAdmin data)
        {
            //var result = (dynamic)null;
            var excludedTaskIds = new List<int> { 10, 11, 12 };
            var emplist = await _context.TblUsersModels.Where(a=> a.DeleteFlag == false).ToListAsync();
            for (int x = 0; x < emplist.Count; x++)
            {
                var empid = emplist[x].EmployeeId;
            
                var existingRecord = await _context.TblPayslipModel.Where(a => a.EmployeeId == emplist[x].EmployeeId.ToString()).ToListAsync();
                ...
                var totalSalary = renderedHours * decimal.Parse(emplist[x].Rate);
                if (!decimal.TryParse(totalSalary.ToString(), out decimal grossSalary))
                    throw new InvalidOperationException("Employee salary rate is invalid.");
